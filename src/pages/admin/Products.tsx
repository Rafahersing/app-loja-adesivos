...
// Interface para a categoria - CORRIGIDA para usar 'name'
interface Category {
  id: string;
  name: string; // <-- CORRIGIDO: Deve ser 'name' para coincidir com utils.ts
  descricao?: string;
}
...
    } else if (data) {
      // CORREÇÃO AQUI: Usando 'cat.name' em vez de 'cat.nome'
      const categoryIdToName: { [key: string]: string } = categories.reduce((map, cat) => {
        map[cat.id] = cat.name; // <--- CORRIGIDO
        return map;
...
      // CORREÇÃO AQUI: Usando cat.name
      const categoryMap: { [key: string]: string } = categories.reduce((map, cat) => {
        map[cat.name.toLowerCase()] = cat.id; // <--- CORRIGIDO
        return map;
      }, {} as { [key: string]: string });
...
                                }}
                              />
                              <label className="text-sm">{cat.name}</label> {/* <--- CORRIGIDO */}
                            </div>
                          ))}
                        </div>
                      </div>
